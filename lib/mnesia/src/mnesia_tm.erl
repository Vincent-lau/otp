...
-export([start/0, init/1, non_transaction/5, transaction/6, commit_participant/6, dirty/2,
         display_info/2, do_update_op/3, get_info/1, get_transactions/0, info/1, mnesia_down/1,
...
-record(prep,
        {protocol = sym_trans,
         ...
         sync = false}).
-record(participant,
        {tid, pid, commit, disc_nodes = [], ram_nodes = [], protocol = sym_trans}).
...
doit_loop(#state{coordinators = Coordinators,
    ...
    receive
        ...
        {From, {ask_commit, Protocol, Tid, Commit0, DiscNs, RamNs}} ->
            ?eval_debug_fun({?MODULE, doit_ask_commit}, [{tid, Tid}, {prot, Protocol}]),
            mnesia_checkpoint:tm_enter_pending(Tid, DiscNs, RamNs),
            Commit = new_cr_format(Commit0),
            Pid = if node(Tid#tid.pid) =:= node() ->
                ...
                {value, P} ->
                    ...
                    case P#participant.pid of
                        nopid ->
                            ...
                            if P#participant.protocol == sync_sym_trans ->
                ...
                {value, P} ->
                    case P#participant.pid of
                        ...
                        Pid when is_pid(Pid) ->
                            ...
                            ?eval_debug_fun({?MODULE, do_abort, post}, [{tid, Tid}, {pid, Pid}]),
                            doit_loop(State)
                    end
            end;
        {From, {add_store, Tid}} -> %% new store for nested  transaction
            try ?ets_new_table(mnesia_trans_store, [bag, public]) of
                Etab ->
                    ...
                    reply(From, {new_store, Etab}, State#state{coordinators = A2})
            catch
                error:Reason -> %% system limit
        ...
        {From, {unblock_tab, Tab}} ->
            ...
            case lists:member(Tab, BlockedTabs2) of
                false ->
                    ...
                    State2 = State#state{blocked_tabs = BlockedTabs2, dirty_queue = Queue},
                    reply(From, ok, State2);
                true ->
...
recover_coordinator(Tid, Etabs) ->
    ...
    try arrange(Tid, Store, async) of
        ...
        _:Reason:Stacktrace ->
            dbg_out("Recovery of coordinator ~p failed: ~tp~n", [Tid, {Reason, Stacktrace}]),
            Protocol = asym_trans,
...
clear_fixtable([Store | _]) ->
    ...
    lists:foreach(fun({Tab, Node}) -> rpc:cast(Node, ?MODULE, fixtable, [Tab, false, self()])
                  end,
                  Fixed).
    ...
    case mnesia_lib:key_search_delete(Node, 1, FT0) of
        ...
        {{Node, Tabs}, FT} ->
            lists:foreach(fun(Tab) ->
                             case ?catch_val({Tab, storage_type}) of
                                 {'EXIT', _} -> ignore;
                                 Storage -> mnesia_lib:db_fixtable(Storage, Tab, false)
                             end
                          end,
                          Tabs),
        ...
        {{Node, Tabs0}, FT} ->
            case lists:delete(Tab, Tabs0) of
                [] ->
                    State#state{fixed_tabs = FT};
                Tabs ->
                    State#state{fixed_tabs = [{Node, Tabs} | FT]}
            end
    end.
...
non_transaction(OldState, Fun, Args, ActivityKind, Mod) ->
    ...
    after
        case OldState of
            ...
            _ ->
                put(mnesia_activity_state, OldState)
        end
    end.
...
execute_inner(Mod, Tid, OldMod, Ts, Fun, Args, Factor, Retries, Type) ->
    case req({add_store, Tid}) of
        ...
        {new_store, Ets} ->
            ...
            NewTs =
                Ts#tidstore{level = 1 + Ts#tidstore.level,
                            store = Ets,
                            up_stores = Up},
...
try_again(infinity) ->
    yes;
try_again(X) when is_number(X), X > 1 ->
    yes;
try_again(_) ->
    no.
...
arrange(Tid, Store, Type) ->
    ...
    Prep =
        case Type of
            async ->
                #prep{protocol = sym_trans, records = Recs};
            sync ->
                #prep{protocol = sync_sym_trans, records = Recs}
        end,
...
reverse([H = #commit{ram_copies = Ram,
                     ...
                     ext = Ext}
         | R]) ->
    [H#commit{ram_copies = lists:reverse(Ram),
...
multi_commit(sym_trans, _Maj = [], Tid, CR, Store) ->
    ...
    ?eval_debug_fun({?MODULE, multi_commit_sym}, [{tid, Tid}, {outcome, Outcome}]),
    ...
    ?eval_debug_fun({?MODULE, multi_commit_sym, post}, [{tid, Tid}, {outcome, Outcome}]),
    Outcome;
multi_commit(sync_sym_trans, _Maj = [], Tid, CR, Store) ->
    ...
    {Outcome, []} = rec_all(WaitFor, Tid, do_commit, []),
    ?eval_debug_fun({?MODULE, multi_commit_sym_sync}, [{tid, Tid}, {outcome, Outcome}]),
...
commit_participant(Protocol, Coord, Tid, Bin, C0, DiscNs, _RamNs) ->
    ...
    try mnesia_schema:prepare_commit(Tid, C0, {part, Coord}) of
        {Modified, C = #commit{}, DumperMode} ->
            ...
            receive
                {Tid, pre_commit} ->
                    ...
                    ExpectAck = C#commit.schema_ops /= [] orelse Protocol =:= sync_asym_trans,
                    ...
                    receive
                        ...
                        {Tid, {do_abort, _Reason}} ->
                            ...
                            ?eval_debug_fun({?MODULE, commit_participant, log_abort}, [{tid, Tid}]),
        ...
        _:Reason ->
            ?eval_debug_fun({?MODULE, commit_participant, vote_no}, [{tid, Tid}]),
...
do_snmp(Tid, [Head | Tail]) ->
    try
        ...
        _:Reason:ST ->
            ...
            %% the snmp table
            verbose("do_snmp in ~w failed: ~tp -> {'EXIT', ~tp}~n", [Tid, Head, {Reason, ST}])
    end,
    do_snmp(Tid, Tail).
...
async_send_dirty(Tid, [Head | Tail], Tab, ReadNode, WaitFor, Res) ->
    ...
    if ReadNode == Node, Node == node() ->
       ...
       true ->
           if node() == 'a@127.0.0.1' andalso Node == 'c@127.0.0.1' ->
                  spawn(fun() ->
                           timer:sleep(1000),
...
new_cr_format(#commit{ext = []} = Cr) ->
    Cr;
new_cr_format(#commit{ext = [{_, _} | _]} = Cr) ->
    Cr;
...
display_info(Stream, {timeout, T}) ->
    io:format(Stream,
              "---> No info about coordinator and participant transactions, "
              "timeout ~p <--- ~n",
              [T]);
...
pr_tid(Stream, Tid) ->
    io:format(Stream, "Tid: ~p (owned by ~p) ~n", [Tid#tid.counter, Tid#tid.pid]).
...
display_pid_info(Pid) ->
    case rpc:pinfo(Pid) of
        ...
        Info ->
            ...
            Curr =
                case fetch(current_function, Info) of
...
reconfigure_participants(N, [P | Tail]) ->
    case lists:member(N, P#participant.disc_nodes) or lists:member(N, P#participant.ram_nodes)
    of
        false ->
        ...
        true ->
            ...
            if node(Tid#tid.pid) /= N ->
               ...
               true ->
                   ...
                   Nodes = P#participant.disc_nodes ++ P#participant.ram_nodes,
                   AliveNodes = Nodes -- [N],
...
system_code_change(State = #state{coordinators = Cs0, participants = Ps0},
                   _Module,
                   _OldVsn,
                   downgrade) ->
...
system_code_change(State = #state{coordinators = Cs0, participants = Ps0},
    ...
    case is_list(Cs0) of
        true ->
            Cs = gb_trees:from_orddict(
                     lists:sort(Cs0)),
            Ps1 = [{P#participant.tid, P} || P <- Ps0],
            Ps = gb_trees:from_orddict(
                     lists:sort(Ps1)),
            {ok, State#state{coordinators = Cs, participants = Ps}};
        false ->
            {ok, State}
